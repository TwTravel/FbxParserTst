...
void DrawSkeletonLocal(FbxNode *node, FbxTime currTime, FbxAnimLayer *currAnimLayer, FbxAMatrix &parentGlobalMatrix)
{
	FbxAMatrix globalPosition = parser->GetGlobalPosition(node, currTime, &parentGlobalMatrix);
	FbxAMatrix localPosition  = parser->GetLocalPosition(node, currTime, &parentGlobalMatrix);
	
	glPushMatrix();

	if (node->GetNodeAttribute())
	{
		FbxVector4  localRotVec  = localPosition.GetR();
		FbxVector4  localTranVec = localPosition.GetT();
		...
		printf("%s , %s , %.2lf, %.2lf ,%.2lf, %.2lf, %.2lf ,%.2lf\n", 
			node->GetParent()->GetName(), node->GetName(),
			localTranVec[0],  localTranVec[1] , localTranVec[2],
			 localRotVec[0],   localRotVec[1] ,  localRotVec[2]);

		FbxVector4 v1  =     (GLdouble*)  parentGlobalMatrix.GetT();
		FbxVector4 v2  =     (GLdouble*)   globalOffPosition.GetT();
        
		double diff_dist = sqrt( (v1[0] - v2[0])*(v1[0] - v2[0]) + 
			                     (v1[1] - v2[1])*(v1[1] - v2[1]) +
							     (v1[2] - v2[2])*(v1[2] - v2[2]) );

		FbxVector4 VV1(0, 0, 0, 0);
		FbxVector4 VV2 = -localTranVec;// (0, 0, -diff_dist, 0);

		glTranslatef(localTranVec[0], localTranVec[1], localTranVec[2]);
		...
		glRotatef(localRotVec[2], 0, 0, 1);
		glRotatef(localRotVec[1], 0, 1, 0);
		glRotatef(localRotVec[0], 1, 0, 0);
	...
	/*	FbxSkeleton *skeleton = (FbxSkeleton*)node->GetNodeAttribute();
		...
		//	node->GetParent()->GetNodeAttribute()->GetAttributeType() == FbxNodeAttribute::eSkeleton)
		{
			...
			glPushMatrix();
			FbxVector4  TTvec = parentGlobalMatrix.GetT();
			glTranslatef(TTvec[0], TTvec[1], TTvec[2]);
			GLUquadricObj *pObj = gluNewQuadric();
			gluSphere(pObj, 5.0, 26, 13);
			glPopMatrix();
	...
	{
		DrawSkeletonLocal(node->GetChild(i), currTime, currAnimLayer, globalPosition);
	}
...
void Display()
{
	...
	{
		cameraManager->SetCamera(parser->GetFbxScene(), parser->GetCurrentTimeKK(), parser->GetAnimLayer(), parser->GetCameraArray(), windowWidth, windowHeight);
		//DrawModel();
		//glDisable(GL_TEXTURE_2D);

		FbxAMatrix lDummyGlobalPosition;
		FbxTime currentTime = parser->GetCurrentTimeKK();
		...
		glPushMatrix();
		//glRotatef(-75, 1, 0, 0);
		  DrawSkeletonLocal(parser->GetFbxScene()->GetRootNode(), currentTime, currAnimLayer, lDummyGlobalPosition);
		glPopMatrix();
